                        ...
                        <tbody class="text-xs divide-y divide-gray-200 bg-indigo-50">
                            @foreach ($books as $book)
                                    <tr>
                                        ...
                                        <td class="px-2 py-4 whitespace-nowrap">
                                            <img src="{{ asset('book') }}/{{$book->cover_image}}" alt="" class=" h-20 w-20 object-cover">
                                        </td>
                                        ...
                                        <td class="px-2 py-4 text-sm text-gray-500 whitespace-nowrap">
                                            <div class="flex justify-start space-x-1">
                                                ...
                                                {{--@if ($book->borrow->student_id === Auth::user()->id && $book->borrow->book_id == $book->id)
                                                    <livewire:buttons.reservation-book :book="$book" :key="$book->id" />

                                                @endif--}}
                                                @php
                                                    $check = DB::table('borrows')->where('student_id',Auth::user()->id)->where('book_id',$book->id)->get();
                                                @endphp
                                                ...
                                                {{--  نتأكد من انو الطالب ما حجز الكتاب من قبل --}}
